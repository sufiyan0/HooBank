...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
            ...
            <div key={footerLinks.title} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              ...
              <ul className='list-none mt-4'>
                {footerLinks.links.map((link, index) => (
                  <li key={link.name} className={`font-poppins cursor-pointer font-normal text-[16px]  leading-6 text-dimWhite hover:text-secondary ${index !== footerLinks.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                    {link.name}
                  </li>
                ))}
              </ul>
    ...
    <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
      <p className={`font-poppins font-normal text-center text-[16px] leading-6 text-white`}>
